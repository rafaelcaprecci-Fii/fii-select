...
function classifyFund(data) {
  ...
  const properties = data.propertiesAndVacancy?.properties || [];
...
function brickFundAdapter(data, cautions) {
  const propertiesData = data.propertiesAndVacancy || {};
  const properties = propertiesData.properties || [];
  const hasArea = properties.some((property) => finiteNumber(property.area) != null);
  const consolidatedVacancy = finiteNumber(propertiesData.vacancyRate);
  const vacancyConsistent =
    consolidatedVacancy != null && consolidatedVacancy > 0 && consolidatedVacancy <= 1;
  const rankedProperties = rankItems(properties, ["revenueShare", "area"]);
  const mainProperties = properties.length > 10 ? rankedProperties.slice(0, 5) : rankedProperties;

  if (hasArea || finiteNumber(propertiesData.totalArea) != null) cautions.push(AREA_CAUTION);
  if (
    consolidatedVacancy != null ||
    properties.some((property) => finiteNumber(property.vacancyRate) != null)
  ) {
    cautions.push(VACANCY_CAUTION);
  }
  if (consolidatedVacancy != null && !vacancyConsistent) cautions.push(MANAGER_REPORT_CAUTION);

  return {
    propertyCount: firstDefined(propertiesData.count, properties.length || null),
...
export function normalizeCrossedReading({ ticker, data }) {
  ...
  if (type === "tijolo") typeSpecific = brickFundAdapter(data, cautions);
  ...
  if (type === "desconhecido") cautions.push(MANAGER_REPORT_CAUTION);
